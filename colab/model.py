...
class SUPPORT(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels=[16, 32, 64, 128, 256], depth=5,\
        ...
        self.model_size = nn.Parameter(torch.Tensor([int(self.mid_channels[0] / 4)]))
...
if __name__ == "__main__":
    ...
    model = SUPPORT(61, mid_channels=[32, 64, 128, 256], depth=4, blind_conv_channels=128, one_by_one_channels=[32, 16], last_layer_channels=[64, 32, 16], bs_size=1, bp=False)
    print(model)
    ...
    inp = torch.zeros(1, 61, 256, 256)
    inp[:, 29, 128, 128] = 1

    out = model(inp)

    plt.imshow(out[0, 0].log().detach().numpy())
    plt.show()
    ...
    if False:
        ...
        model = SUPPORT(in_channels=ch, mid_channels=[16, 32, 64, 128, 256], depth=3,\
            blind_conv_channels=4, one_by_one_channels=[32, 16],\
                    last_layer_channels=[4, 1], bs_size=3, bp=False)
        model.apply(weights_init_normalized)

        import numpy as np

        inp = torch.zeros(1, 1, 1024, 1024)
        inp[:, :, 500, 500] = 100
        out = model(inp)
